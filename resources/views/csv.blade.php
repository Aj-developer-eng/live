...
<form action="{{ URL::to('') }}/upload_csv" method="post" enctype="multipart/form-data">
    @csrf
...
<script>
    ...
    extendedTimeOut: 0, // Disable extended time-out

            positionClass: "toast-center",
            onShown: function () {
                // Create custom HTML content
                var customContent = '<div class="custom-notification">' +
                    '<h2>Your Csv Is Being Upload!</h2>' +
                    '<div class="progress">' +
                    '<div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuenow="75" aria-valuemin="0" aria-valuemax="100" style="width: 75%"></div>' +
                    '</div>' +
                    '</div>';

                // Replace the Toastr notification content with custom content
                $('.toast').html(customContent);
            }
        };
    ...
    @endif
    {{--  console.log('Before listening to channel');
    window.Echo.channel('example-channel')
        .listen('ExampleEvent', (event) => {
            console.log('Received event:', event);
            // Handle the event data
        });
    console.log('After listening to channel');  --}}
  </script>
  <script src="//cdnjs.cloudflare.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>
    ...
    <script type="text/javascript">
      ...
      var pusher = new Pusher('8de71e3cf78e48a5ce12', {
        encrypted: true,
        cluster: 'ap2'
      });
